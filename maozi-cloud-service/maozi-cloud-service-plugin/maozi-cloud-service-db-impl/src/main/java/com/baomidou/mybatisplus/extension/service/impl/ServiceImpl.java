...
public class ServiceImpl<M extends BaseMapper<T>, T extends AbstractBaseDomain, E extends AbstractBaseCode> extends BaseCommon<E> implements IService<T> {
    ...
    public boolean saveOrUpdateBatch(Collection<T> entityList, int batchSize) {
        ...
        String keyProperty = tableInfo.getKeyProperty();
        Assert.notEmpty(keyProperty, "error: can not execute. because can not find column for id from entity!");
        return SqlHelper.saveOrUpdateBatch(this.entityClass, this.mapperClass, this.mybatisLog, entityList, batchSize, (sqlSession, entity) -> {
    ...
    public T getOne(Wrapper<T> queryWrapper, boolean throwEx) {
        ...
        return SqlHelper.getObject(mybatisLog, baseMapper.selectList(queryWrapper));
    }

    @Override
    public Map<String, Object> getMap(Wrapper<T> queryWrapper) {
        return SqlHelper.getObject(mybatisLog, baseMapper.selectMaps(queryWrapper));
    }

    @Override
    public <V> V getObj(Wrapper<T> queryWrapper, Function<? super Object, V> mapper) {
        return SqlHelper.getObject(mybatisLog, listObjs(queryWrapper, mapper));
    }
    ...
    protected boolean executeBatch(Consumer<SqlSession> consumer) {
        return SqlHelper.executeBatch(this.entityClass, this.mybatisLog, consumer);
    }
    ...
    protected <E> boolean executeBatch(Collection<E> list, int batchSize, BiConsumer<SqlSession, E> consumer) {
        return SqlHelper.executeBatch(this.entityClass, this.mybatisLog, list, batchSize, consumer);
    }
